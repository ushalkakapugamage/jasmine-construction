  ...
  return <PageLayout headerDark>
    ...
    <section className="home-hero" style={{ backgroundImage: `linear-gradient(90deg, rgba(8,46,91,.95) 0%, rgba(8,46,91,.76) 47%, rgba(23,33,43,.38) 100%), url(${heroImage})` }}>
      <div className="shell home-hero__content">
        ...
        <div className="hero-actions"><Button href="/projects">Explore projects</Button><Button href="/contact" variant="line">Start your project</Button></div>
      </div>
    ...
    <section className="archive-section">
      <div className="shell">
        ...
        {publishedProjects.length > 0 ? <div className="project-grid">{publishedProjects.slice(0, 3).map((project) => <a className="project-card" href={`/projects/${project.slug}`} key={project.id}><img src={project.coverImage} alt={project.name} loading="lazy" /><span className="project-card__meta">{project.category} · {project.location}</span><h3>{project.name}</h3><ArrowUpRight /></a>)}</div> : <div className="archive-preview">
        ...
        <div className="archive-section__footer"><p>Project names, locations, imagery and scope will appear only after confirmation.</p><Button href="/projects" variant="line">View project archive</Button></div>
      </div>
    </section>
    ...
    <section className="approach-section shell">
      ...
      <div className="approach-section__content"><p className="eyebrow">A more considered first step</p><h2>Start with the right questions.</h2><p>Every brief has its own practical context. Share the site, your intent and where you are in the process. We’ll help make the next conversation more focused.</p><Button href="/contact">Discuss your project</Button></div>
    </section>

    <section className="process-section">
      <div className="shell"><SectionIntro number="03" label="How to begin" title="A clear route from enquiry to next step." /><div className="process-grid">{processOutline.map(([number, title, copy]) => <article key={number}><span>{number}</span><h3>{title}</h3><p>{copy}</p></article>)}</div><div className="process-section__footer"><p>The detailed delivery process will be confirmed and published with Jasmin Constructions’ approved operating information.</p><Button href="/process" variant="line">See the process</Button></div></div>
    </section>

    <section className="closing-cta"><div className="shell closing-cta__inner"><p className="eyebrow eyebrow--light">Start a conversation</p><h2>Planning a project?</h2><p>Tell Jasmin Constructions what you are considering, and make the first step a clear one.</p><div><Button href="/contact" variant="light">Start your project</Button><a className="plain-link" href={`https://wa.me/${site.whatsapp}?text=Hello%20Jasmin%20Constructions%2C%20I%27d%20like%20to%20discuss%20a%20project.`} target="_blank" rel="noreferrer">WhatsApp us <ArrowUpRight /></a></div></div></section>
  </PageLayout>
}
