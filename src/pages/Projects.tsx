...
export function Projects() {
  ...
  return <PageLayout headerDark><Seo title="Projects" description="A verified project archive from Jasmin Constructions." /><PageHero eyebrow="Project archive" title="Work with a record behind it." text="Each published project will be presented with verified details, relevant scope and original photography." image={hero} />
    <section className="page-section shell"><SectionIntro number="01" label="Verified work" title="Projects in context." text="The archive is intentionally held until project names, photography and information can be verified for publication." />
      ...
      {visible.length ? <div className="project-grid project-grid--archive">{visible.map((project) => <a className="project-card" href={sitePath(`/projects/${project.slug}`)} key={project.id}><img src={project.coverImage} alt={project.name} loading="lazy" /><span className="project-card__meta">{project.category} · {project.location}</span><h2>{project.name}</h2><ArrowUpRight /></a>)}</div> : <EmptyArchive />}
    </section>
    <section className="closing-cta"><div className="shell closing-cta__inner"><p className="eyebrow eyebrow--light">Have a project in mind?</p><h2>Start with a considered brief.</h2><p>We’ll make it simple to share the details that matter.</p><Button href={sitePath('/contact')} variant="light">Start your project</Button></div></section>
  </PageLayout>
}

function EmptyArchive() {
  return <div className="empty-archive"><div className="empty-archive__number">/01</div><div><p className="eyebrow">Archive in preparation</p><h2>Verified project records are being prepared.</h2><p>Jasmin Constructions will add each project only with approved descriptions, images and client information where publishable.</p><Button href={sitePath('/contact')} variant="line">Discuss a similar project</Button></div></div>
}
